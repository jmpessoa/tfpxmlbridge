...
uses
  Classes, SysUtils, DOM, XMLWrite, XMLRead, TypInfo;
...
type
  ...
  TFPXMLBridge = class
    ...
    public
      {public declarations }
      WhenManyTextContentShowNodeName: boolean;
      Active: boolean;

      NamespaceSeparatorToken: char;
      AssignmentDelimiterToken: Char;
      NameValueSeparatorToken: char;
      AssignmentToken: char;
      AttributeNameValueStartToken: char;
      AttributeNameValueEndToken: char;
      AttributesSeparatorToken: char;
      ConcatenationToken: char;
      IndexStartToken: char;
      IndexEndToken: char;
      BridgeLateBindingToken: char;
      ...
      function GetChildByName(referenceNode: TDOMNode; name: string): TDOMNode;
      function GetChildNodeByNameAndAttribute(refNode:TDOMNode; nodeName, attrValue:string): TDOMNode;

      function GetValueFromChildByName(referenceNode: TDOMNode; name: string): string;
...
constructor TFPXMLBridge.Create;
begin
  inherited;
...
function TFPXMLBridge.GetChildByName(referenceNode: TDOMNode; name: string): TDOMNode;
begin
   Result:=nil;
   if referenceNode.NodeType = ELEMENT_NODE then
   begin
      Result:=getNodeFromNodeRerence(referenceNode, referenceNode.NodeName+ NamespaceSeparatorToken{'.'}+name)
   end;
   FCurrentNode:= Result;
end;

function TFPXMLBridge.GetValueFromChildByName(referenceNode: TDOMNode; name: string): string;
begin
   Result:='';
   if referenceNode.NodeType = ELEMENT_NODE then
   begin
      Result:=getValueFromNodeRerence(referenceNode, referenceNode.NodeName+ NamespaceSeparatorToken{'.'}+name)
   end;
end;
